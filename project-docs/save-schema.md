...
3. **Склад (Storage-модуль)** — `construction[module-type=ModuleTypeStorage] >
   ...
   ```
   - Слот может держать **один ресурс ИЛИ стопку** — тогда `<resource>` становится
     массивом (fast-xml-parser вернёт `slot.resource` как `ResourceNode[]`). Высота стопки
     ограничена `max-height`. **Любой обход `slot.resource` оборачивать в `asArray()`.**
   - Пустой слот — без дочернего `<resource>`.
   - **Консервативный лимит для инъекции = число ПУСТЫХ слотов** (size-3 ≈ 49 слотов,
     часть занята). Реальная ёмкость выше за счёт стекинга, но безопасно класть по одному
     в пустые слоты. Стекинг до `max-height` — потенциальное улучшение (не сделано в v1).
   - Добавление: найти пустой слот, вставить `<resource>` с `position` = позиция слота,
     `location=0`, `state=2`. `max-height` не трогаем.
