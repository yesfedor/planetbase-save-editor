...
3. **Склад (Storage-модуль)** — `construction[module-type=ModuleTypeStorage] >
   ...
   ```
   - Слот — это **вертикальная СТОПКА**: `slot.resource` бывает объектом ИЛИ массивом
     (`ResourceNode[]`). **Любой обход оборачивать в `asArray()`.** Пустой слот — без `<resource>`.
   - **Ёмкость слота = «бюджет высоты» `max-height`** (у каждого слота свой, 0.5–4.65;
     выше к центру купола). Каждый ресурс занимает высоту:
     - **0.9 (целая):** Metal, Bioplastic, Spares, Ore(предпол.)
     - **0.45 (половина):** Meal, AlcoholicDrink, MedicalSupplies, Gun, MedicinalPlants,
       Semiconductors, Vegetables, Vitromeat, Starch(предпол.)
     (измерено по y-дельтам между ресурсами в стопке на save2.sav/quick1.sav)
   - **Реальная ёмкость склада для типа X** = Σ_слотов `floor(max-height_слота / высота(X))`.
     Пример size-3 (49 слотов, Σmax-height≈96.6): **~80 целых** или **~189 половинных** —
     НЕ «49». Проверено: заполненный склад save2 ≈ 100% (Σused-height ≈ Σmax-height).
   - `y` каждого ресурса в стопке = базовый `slot.position.y` + накопленная высота ниже.
   - Заполнение (`fillStorage`): по слотам стекингом до `max-height`, y инкрементим на высоту.
   - **⚠️ Старая логика «1 ресурс = 1 слот» была неверной** — исправлено в `core/services/storage.ts`.
